...
namespace UniCore.API.Controllers.v1
{
    ...
    public class StudentController : BaseController
    {
        ...
        [ProducesResponseType(typeof(BaseAPIResponse<IEnumerable<GetClassFriendsResponseDTO>>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<BaseAPIResponse<IEnumerable<GetClassFriendsResponseDTO>>>> GetClassmates(
            [FromQuery] string? classId,
            CancellationToken ct = default)
        {
            if (string.IsNullOrEmpty(classId))
            {
                var errMessage = _localizer.GetString(MessageConstants.Auth.IdentityNotFound);
                return BadRequestResponse<IEnumerable<GetClassFriendsResponseDTO>>(errMessage);
            }
            ...
            {
                var errMessage = _localizer.GetString(MessageConstants.Auth.IdentityNotFound);
                return UnauthorizedResponse<IEnumerable<GetClassFriendsResponseDTO>>(errMessage);
            }
            ...
            {
                var notFoundMessage = _localizer.GetString(MessageConstants.Auth.UserNotFound);
                return NotFoundResponse<IEnumerable<GetClassFriendsResponseDTO>>(notFoundMessage);
            }

            var successMessage = _localizer.GetString(MessageConstants.Auth.GetMeSuccess);
            return OkResponse<IEnumerable<GetClassFriendsResponseDTO>>(result, successMessage);
